...
function Identity(userID, passphrase, keyPair) {
  ...
  this._publicKeyArmored = keyPair.publicKey.toString();
  this._privateKeyArmored = keyPair.privateKey.toString();

  this.userID = userID;
  this.publicKey = pgp.key.readArmored(this._publicKeyArmored).keys[0];
  this.privateKey = pgp.key.readArmored(this._privateKeyArmored).keys[0];
...
* #getPubKeyHash
*/
Identity.prototype.getPubKeyHash = function() {
  var pubkeyhash = crypto.createHash('sha1').update(this._publicKeyArmored);

  return pubkeyhash.digest('hex');
};

/**
* Returns the assumed href for the pubkey
* #getPubKeyHref
*/
Identity.prototype.getPubKeyHref = function() {
  var host = utils.getPodHostFromUserID(this.userID);
  var alias = utils.getAliasFromUserID(this.userID);

  return ['https://', host, '/aliases/', alias].join('');
};
