...
Session.prototype.playback = function(callback) {
  ...
  self.client.createToken(function(err, result) {
    if (err) {
      return callback(err);
    }

    self.client.getMissedMessages(result.token, function(err, messages) {
      if (err) {
        return callback(err);
      }

      callback(null, messages, self._purge);
    });
  });
};
...
Session.prototype._purge = function(callback) {
  ...
  self.client.createToken(function(err, result) {
    ...
    self.client.purgeMessageHistory(result.token, function(err, messages) {
      if (err) {
        return callback(err);
      }
